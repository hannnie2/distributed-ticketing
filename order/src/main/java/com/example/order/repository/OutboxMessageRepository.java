...
public interface OutboxMessageRepository extends JpaRepository<OutboxMessage, Long> {
    ...
    @Modifying
    @Query("UPDATE OutboxMessage m SET m.status = com.example.order.constants.OutboxMessageStatus.PENDING WHERE m.status = com.example.order.constants.OutboxMessageStatus.PROCESSING AND m.updatedAt < :cutoff")
